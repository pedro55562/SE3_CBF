...
def compute_distance_hessian_analytical( 
    ...
    dist, _, a_star, b_star = compute_distance_gradient(
        robot_ob, ob, curr_state, curr_jac, dist_param_h, dist_param_eps
    )


    a_star = jnp.asarray(a_star).reshape(3,)
    b_star = jnp.asarray(b_star).reshape(3,)
    dist = float(dist)

    J_Pi_A = jnp.asarray(
        jacobian_projection_dispatch(robot_ob, b_star, dist_param_h, dist_param_eps)
    )
    J_Pi_B = jnp.asarray(
        jacobian_projection_dispatch(ob, a_star, dist_param_h, dist_param_eps)
    )
    ...
    H_A = jnp.eye(3) - J_Pi_A
    H_B = jnp.eye(3) - J_Pi_B
    ...
    v_A = xi[:3].reshape(3,)
    omega_A = xi[3:6].reshape(3,)
    s_A = jnp.asarray(curr_state[:3, 3]).reshape(3,)

    r_bs = b_star - s_A
    r_ba = b_star - a_star

    psi_A = (v_A + jnp.cross(omega_A, r_bs)).reshape(3,)
    W = jnp.cross(omega_A, r_ba).reshape(3,)
    ...
    rhs = (H_A @ psi_A - W).reshape(3,)

    a_star_dot = jnp.linalg.solve(M, rhs).reshape(3,)
    b_star_dot = ((jnp.eye(3) - H_B) @ a_star_dot).reshape(3,)

    d_ab_dot = (a_star_dot - b_star_dot).reshape(3,)


    n = ((a_star - b_star) / dist).reshape(3,)

    proj = jnp.dot(n, d_ab_dot)
    n_dot = ((d_ab_dot - n * proj) / dist).reshape(3,)


    curr_jac = jnp.asarray(curr_jac)

    J_v = curr_jac[:3, :] 

    jac_dist_dot = J_v.T @ n_dot.reshape(3, 1)

    return np.array(jac_dist_dot)
...
if simular_movimento:
    ...
    for k in range(int(t_max / dt)):
        ...
        for ob in all_obs:
            ...
            hess_dist = compute_distance_hessian_analytical(robot_ob, ob, xi, curr_state, curr_jac, dist_param_h, dist_param_eps)
            hess_dist = hess_dist.reshape(-1)
        ...
        try:
            ...
            print("\n QP Falhou!  ")
            print("Tempo: ", t)
            sim.save(address="/home/pedro/Projects/SE3_CBF/se3_rigid_body_second_order",
...
sim.save(
    address="/home/pedro/Projects/SE3_CBF/se3_rigid_body_second_order",
    file_name="se3_teste",
)